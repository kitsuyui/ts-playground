...
describe('from2bitArrayTo128bit', () => {
  it('throws when the array length is not 64', () => {
    expect(() => from2bitArrayTo128bit(new Uint8Array(63))).toThrow(
      'Array length must be 64'
    )
  })

  it('throws when a value is out of the 2-bit range', () => {
    const input = new Uint8Array(64)
    input[3] = 4

    expect(() => from2bitArrayTo128bit(input)).toThrow('Invalid bit value: 4')
  })
})
